...
<div class="formbody">
	<div id="usual1" class="usual">
		<div class="itab">
			<ul>
				<li><a href="#tab1" class="selected">签到查询</a></li>
			</ul>
		</div>

		<div id="tab1" class="tabson">
			<form action="${pageContext.request.contextPath}/queryTeaCommonRecords.do" method="post" name="form4" id="form4">
				<ul class="seachform">
					<li><label>班级名称</label><input type="text" name="commonClassName" size="10" value="${param.commonClassName}" class="scinput" /></li>
					<li><label>教师姓名</label><input type="text" name="teaManName" size="10" value="${param.teaManName}" class="scinput" /></li>
			...
			<table class="tablelist">
				...
				<c:forEach items="${list}" var="teaCommonRecords">
				<tbody>
				<tr>
					<td>${teaCommonRecords.teaManName}</td>
					<td>${teaCommonRecords.teaManPhone}</td>
					<td>${teaCommonRecords.classTypeName}</td>
					<td>${teaCommonRecords.commonClassName}</td>
					<td>${teaCommonRecords.commonCost}</td>
					<td>${teaCommonRecords.commonSpan}</td>
					<td>
					<fmt:formatDate value="${teaCommonRecords.teaCommonCardTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
					</td>
					<td>
						<c:choose>
                            <c:when test="${teaCommonRecords.teaCommonStatus==0}">未签到</c:when>
                            <c:when test="${teaCommonRecords.teaCommonStatus==1}">已签到</c:when>
                        </c:choose>
					</td>
				</tr>
				</c:forEach>
	...
	<div class="pagin">
		...
		<form action="${pageContext.request.contextPath }/queryTeaCommonRecords.do" method="post">
		...
		<form action="quertyteacher.do" method="post">
			<ul class="paginList">
				<c:if test="${currentPageNo>1}">
					<li class="paginItem"><a href="${pageContext.request.contextPath }/queryTeaCommonRecords.do?currentPageNo=1&pageSize=${param.pageSize}&commonClassName=${param.commonClassName}&teaManName=${param.teaManName}&da1=${requestScope.da1}&da2=${requestScope.da2}">首页</a></li>
					<li class="paginItem"><a href="${pageContext.request.contextPath }/queryTeaCommonRecords.do?currentPageNo=${currentPageNo-1}&pageSize=${param.pageSize}&commonClassName=${param.commonClassName}&teaManName=${param.teaManName}&da1=${requestScope.da1}&da2=${requestScope.da2}" class="pia">上一页</a></li>
                </c:if>
				<c:if test="${currentPageNo<=1}">
				...
				<li class="paginItem"><a href="${pageContext.request.contextPath }/queryTeaCommonRecords.do?currentPageNo=${currentPageNo+1}&pageSize=${param.pageSize}&commonClassName=${param.commonClassName}&teaManName=${param.teaManName}&da1=${requestScope.da1}&da2=${requestScope.da2}" class="pia">下一页</a></li>
				<li class="paginItem"><a href="${pageContext.request.contextPath }/queryTeaCommonRecords.do?currentPageNo=${totalPageCount}&pageSize=${param.pageSize}&commonClassName=${param.commonClassName}&teaManName=${param.teaManName}&da1=${requestScope.da1}&da2=${requestScope.da2}">尾页</a></li>
				</c:if>
                <c:if test="${currentPageNo>=totalPageCount}">
