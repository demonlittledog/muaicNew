...
<div class="formbody">
	<div id="usual1" class="usual">
		...
		<div id="tab1" class="tabson">
			<form action="${pageContext.request.contextPath}/queryCommonRecords.do" method="post" name="form4" id="form4">
				<ul class="seachform">
			...
			<table class="tablelist">
                ...
                <c:forEach items="${list}" var="commonRecords">
				<tbody>

				<tr>
					<td>${commonRecords.studentName}</td>
					<td>${commonRecords.parentsPhone}</td>
					<td>${commonRecords.classTypeName}</td>
					<td>${commonRecords.commonClassName}</td>
					<td>${commonRecords.teaManName}</td>
					<td>${commonRecords.commonCost}</td>
					<td>${commonRecords.commonSpan}</td>
					<td>
						<fmt:formatDate value="${commonRecords.commonCardTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
					</td>
					<td>
						<c:choose>
                            <c:when test="${commonRecords.commonStatus==0}">未签到</c:when>
                            <c:when test="${commonRecords.commonStatus==1}">已签到</c:when>
	...
	<div class="pagin">
		...
		<form action="${pageContext.request.contextPath }/queryCommonRecords.do" method="post">
		...
		<form action="quertyteacher.do" method="post">
			<ul class="paginList">
				<c:if test="${currentPageNo>1}">
					<li class="paginItem"><a href="${pageContext.request.contextPath }/queryCommonRecords.do?currentPageNo=1&pageSize=${param.pageSize}&commonClassName=${param.commonClassName}&studentName=${param.studentName}&da1=${requestScope.da1}&da2=${requestScope.da2}">首页</a></li>
					<li class="paginItem"><a href="${pageContext.request.contextPath }/queryCommonRecords.do?currentPageNo=${currentPageNo-1}&pageSize=${param.pageSize}&commonClassName=${param.commonClassName}&studentName=${param.studentName}&da1=${requestScope.da1}&da2=${requestScope.da2}" class="pia">上一页</a></li>
                </c:if>
                <c:if test="${currentPageNo<=1}">
				...
				<li class="paginItem"><a href="${pageContext.request.contextPath }/queryCommonRecords.do?currentPageNo=${currentPageNo+1}&pageSize=${param.pageSize}&commonClassName=${param.commonClassName}&studentName=${param.studentName}&da1=${requestScope.da1}&da2=${requestScope.da2}" class="pia">下一页</a></li>
				<li class="paginItem"><a href="${pageContext.request.contextPath }/queryCommonRecords.do?currentPageNo=${totalPageCount}&pageSize=${param.pageSize}&commonClassName=${param.commonClassName}&studentName=${param.studentName}&da1=${requestScope.da1}&da2=${requestScope.da2}">尾页</a></li>
				</c:if>
                <c:if test="${currentPageNo>=totalPageCount}">
